...
def main():
    ...
    parser.add_argument(
        "-p", "--port", type=int, help="target port"
    )
    parser.add_argument(
        "-t", "--target_host", type=str, help="target host", default="0.0.0.0")
    parser.add_argument(
        "-l", "--listen", help="listen on [host]:[port] for incoming connections", action="store_true", default=False
    )
    parser.add_argument(
        "-e", "--execute", help="--execute=file_to_run xecute the given file upon receiving a connection"
    )
    parser.add_argument(
        "-c", "--command", help="initialize a command shell", action="store_true", default=False
    )
    parser.add_argument(
        "-u", "--upload", help="--upload=destination upon receiving connection upload a file and write to [destination]"
    )
...
def run_command(com):
    # trim the newline
    com = com.rstrip()
    print("DBG: executing command: " + com)
    ...
    try:
        ...
        output = "Failed to execute command. \015\012"
...
def client_handler(client_socket):
    ...
    if upload_destination is not None:
        ...
        # write bytes to file
        try:
            f = open(str(upload_destination), "wb")
            f.write(bytes(file_buff))
            f.close()

            # ACK file writing
            client_socket.send("Successfully saved file to {0}\015\012".format(upload_destination).encode())
        except:
            client_socket.send(
                "Failed to save file to {0}. Are you sure the directory exists?\015\012".format(upload_destination).encode()
            )
